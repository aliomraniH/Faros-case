# memory/ — MCP_Assist coordination layer

The knowledge tree lives in git (durable, diff-able, reviewable). **MCP_Assist** holds the
...
- **Server name to call:** `MCP_Assist` (tools as `MCP_Assist:memory_save`, etc.)
...
Every MCP_Assist call from the seeding session returned `{"code":32600,"message":"Session
...
MCP_Assist loaded, replay them in order (the `repo/seed-commit` claim should be saved *after* the
push so `coord_reconcile` can verify it).

## Bootstrap sequence (for any surface picking this up)
1. `MCP_Assist:coord_health("dev/faros-case")` — see what's there / what's stale.
2. `MCP_Assist:memory_list` (or `memory_search`) to load the `insight/*` and `arch/*` entries.
3. `MCP_Assist:handoff_load("handoff/web-to-cowork", "dev/faros-case")` — get the next steps.
