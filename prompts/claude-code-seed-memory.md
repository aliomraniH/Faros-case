# Prompt — Claude Code · seed assist-memory memory

The assist-memory connector is configured in `.mcp.json` (server name `assist-memory`, namespace
`dev/faros-case`).

1. Call `mcp__assist-memory__coord_health` for namespace `dev/faros-case`. If it errors with "Session
...
4. After the push exists, save the `repo/seed-commit` claim, then run
   `mcp__assist-memory__coord_reconcile` for `dev/faros-case` and report each verdict
   (current/stale/unverifiable). If the backend has no GitHub token, expect `unverifiable`.
5. Confirm with `mcp__assist-memory__memory_list` that the `insight/*`, `arch/*`, and `open/pricing`
